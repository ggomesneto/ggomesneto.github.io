...
function generateinfoBox(len, charlen) {
	...
	<div class="pw-container" id='pw-info'>
      ...
      <div class="pw-body" id='pw-info-body'>
		...
		<div class="form-control">
    	  <label for="salt">Add Salt:</label>
    	  <input id='salt' type='checkbox'></input>
		</div>
		<buttom class='generate' id='hashit'>Generate Hash</buttom>
...
function generateHashBox() {
	...
	if ($('.hash-log')[0]) {
		if ($('#salt').prop('checked') === true) {
			salt = generateSalt();
			pw = $pw.text() + salt;
			hash = generateHash(pw);
		} else {
			let pw = $pw.text();
			hash = generateHash(pw);
		}
		...
		let markup = `
		<div class='hash-info'>
		<i class="fas fa-long-arrow-alt-right fa-3x"></i> ${hash}
		<div class='close'>X</div>	
		</div>
		`;
		$('.hash-log').append(markup);
	} else {
		if ($('#salt').prop('checked') === true) {
			salt = generateSalt();
			pw = $pw.text() + salt;
	...
	let saltinfo = `
	<div class='salt-info'>Salting is simply the addition of a unique, random string of characters known only to the site to each password before it is hashed. Salting makes each password unique, even when the input password is the same.<br> ( Try Hashing with and without Salting )</div>
	`;

	$('body').append(infobox);

	if ($('#salt').prop('checked') === true) {
		$('.info-hash').append(saltinfo);
	}
}
